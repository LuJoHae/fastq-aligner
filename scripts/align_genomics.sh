...
#SBATCH --output=$HOME/log/%A/star_align_%A_%a.out
#SBATCH --error=$HOME/log/%A/star_align_%A_%a.err
#SBATCH --time=24:00:00
#SBATCH --exclusive
#SBATCH --array=0-589%200
#SBATCH --partition=normal
#SBATCH --mail-user=lukas.haeuser@empa.ch
...
# Skip if library type is not GENOMIC
if [ "$LIBRARY_TYPE" != "GENOMIC" ]; then
    echo "Skipping sample $SAMPLE_ID - library type is $LIBRARY_TYPE (not GENOMIC)"
    exit 0
fi
